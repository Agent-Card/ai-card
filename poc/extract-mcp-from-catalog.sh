...
TITLE_FILTER="${2:-}"
MCP_MEDIA_TYPE="application/vnd.lf.ai.card.mcp.v1+json"
...
mcp_digest="$(printf '%s' "$manifest_json" | jq -r --arg mt "$MCP_MEDIA_TYPE" '
  .layers[] | select(.mediaType == $mt) | .digest
' | head -n 1)"

if [[ -z "$mcp_digest" || "$mcp_digest" == "null" ]]; then
  echo "Selected manifest does not contain an MCP layer ($MCP_MEDIA_TYPE)." >&2
  exit 1
fi

echo "Catalog:          $INDEX_JSON"
echo "Manifest digest:  $manifest_digest"
echo "MCP layer digest: $mcp_digest"
echo
echo "MCP server card JSON:"
oras blob fetch --oci-layout --output - "$OCI_LAYOUT_DIR@$mcp_digest" | jq .
